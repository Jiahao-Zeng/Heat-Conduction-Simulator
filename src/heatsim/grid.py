...
def _material_fields(alpha, k, rho_c, shape):
    ...
    if alpha is not None:
        ...
        k_field = _conductivity_field(alpha, shape)
...
class Grid2D(_MaterialCacheMixin):
    def __init__(self, length_x, length_y, nx, ny, alpha=None,
        ...
        if k_y is not None and alpha is not None:
            raise ValueError(
                "k_y (anisotropic conductivity) cannot be combined with "
                "alpha; pass k instead")
    ...
    def face_k_y(self):
        if self._face_k_y is None:
            self._face_k_y = harmonic_face_mean(self.k_y, axis=1)
